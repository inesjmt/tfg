...
void comunicacion() {
    ...
    CTrackerPod trackerPod = CTrackerPod.CreateTrackerPod();
    ...
    do {
        ...
        if(recibidos != SOCKET_ERROR)
        {
            ...
            sscanf_s(buffer,"%s %d %d %d",comando,&idCliente,&deltaX,&deltaY);

            if (strcmp(comando, "mover") == 0) {
                trackerPod.move_by(deltaX * VELOCIDAD_ROTACION, deltaY * VELOCIDAD_ROTACION);
                /*
                clientes[param1].x=param2;
                clientes[param1].y=param3;
                clientes[param1].z=param4;
                if(abs(clientes[idCliente].x)) {
                    sprintf_s(buffer,sizeof(buffer)," %d\r\n",(short)clientes[idCliente].id);

                } else {
                    sprintf_s(buffer,sizeof(buffer),"mover %d %d %d %d\r\n",(short)clientes[idCliente].id,(short)clientes[idCliente].x,(short)clientes[idCliente].y,(short)clientes[idCliente].z);
                }
                */
            }
